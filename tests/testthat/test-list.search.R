...
test_that("list.search", {
  ...
  expect_identical(list.search(x,exact("A")),list(p1=list(type="A")))
  expect_identical(list.search(x,exact("A"),unlist = TRUE),c(p1.type="A"))
  expect_identical(list.search(x,exact(9)),
  ...
  expect_equal(list.search(x,any(like("Ken",1)),"character",unlist = TRUE),
    c(p1.name="Ken",p2.name="Kent",p5.name="Kwen"))
  expect_identical(list.search(x,all(like("Ken",0)),"character"),
  ...
  expect_equal(list.search(x,all(like("Ken",1)), "character"),
    list(p1=list(name=c("Ken","Ren"))))
  expect_equal(list.search(x,any(like("Ken",1)), "character"),
    list(p1=list(name=c("Ken","Ren")),
